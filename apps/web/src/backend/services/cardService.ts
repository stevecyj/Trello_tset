...
class CardService {
  ...
  restructureCardForChart(groupedCardMap: GroupedCard){
    ...
    for (let card_status in groupedCardMap){
      for (let month in groupedCardMap[card_status]){
          ...
          let label_number_map = groupedCardMap[card_status][month]
